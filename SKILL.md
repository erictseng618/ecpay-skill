...
> 新型閘道同時提供 **7 家銀行**的閘道服務，涵蓋市場約 **80%** 的信用卡，支援 3-24 期消費分期。額外加入 ATM 虛擬帳號、超商條碼與代碼，並完整支援**國旅卡**與**美國運通卡 (AMEX)**。
...
- **不可省略 Callback 回應**：CMV-SHA256 回 `1|OK`、ECPG 回 JSON `{ "TransCode": 1 }`、國內物流 CMV-MD5 回 `1|OK`、全方位/跨境物流 v2 回 **AES 加密 JSON**（三層結構）、電子票證回 **AES 加密 JSON**（三層結構，Data 內 `{"RtnCode": 1, "RtnMsg": "成功"}`）。**`1|OK` 常見錯誤格式**（會導致系統重發 4 次）：`"1|OK"`（含引號）、`1|ok`（小寫 ok）、`_OK`、`1OK`（缺分隔）、帶空白或換行
