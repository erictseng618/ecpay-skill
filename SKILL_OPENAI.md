...
7. **Callback responses differ by protocol**: CMV-SHA256 (AIO) returns `1|OK`; 站內付 2.0 (ECPG checkout) returns `1|OK` (spec 9058.md); credit backend returns `1|OK` (spec 45907.md); non-credit backend returns `1|OK`; logistics v2 returns AES-encrypted JSON; e-ticket returns **AES-encrypted JSON + CheckMacValue** (Data contains `{"RtnCode":1,"RtnMsg":"成功"}`); domestic logistics returns `1|OK`; **B2C invoice AllowanceByCollegiate returns `1|OK`** (⚠️ callback is Form POST + CheckMacValue **MD5**, the only invoice API with CheckMacValue — see guides/04). **Common `1|OK` mistakes** (cause 4 retries): `"1|OK"` (with quotes), `1|ok` (lowercase), `_OK`, `1OK` (no separator), whitespace/newline.
