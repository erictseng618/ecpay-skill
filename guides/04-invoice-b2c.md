...
> ⚠️ **B2C 發票唯一的雙驗證 API**：此 API 的 ReturnURL Callback 包含 `CheckMacValue`（**MD5**），是 B2C 發票中唯一具有 CheckMacValue 驗證的 API。Callback 格式為 Form POST（`application/x-www-form-urlencoded`），非 AES-JSON。CheckMacValue 計算方式與 AIO 金流相同（排序參數 + URL encode + MD5），詳見[檢查碼機制](https://developers.ecpay.com.tw/38242/)。
