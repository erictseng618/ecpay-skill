...
```
CheckMacValue = SHA256(URLEncode(HashKey={HashKey}&Data={Data明文JSON}&HashIV={HashIV}))
```

> **重要**：此公式使用 **Data 明文 JSON**（加密前），不是 AIO 金流的「所有欄位排序拼接」方式。兩者不可混用。詳見 [guides/09 §CheckMacValue 計算](./09-ecticket.md)。
