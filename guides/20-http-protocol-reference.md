...
| 電子票證（3 種模式） | AES-JSON | application/json | AES Data | AES-128-CBC | ecticket.ecpay.com.tw | [09](./09-ecticket.md) |
...
**適用服務**：ECPG 站內付、B2C/B2B 電子發票、信用卡幕後授權、非信用卡幕後取號、全方位物流 v2、跨境物流、電子票證、直播收款、離線電子發票
...
### 2.4 CMV-MD5 — Form POST + CheckMacValue (MD5)
...
| **電子票證** | AES-JSON | `ecticket-stage.ecpay.com.tw` | `ecticket.ecpay.com.tw` | ~6 | [guides/09](./09-ecticket.md) |
...
| AES-JSON（電子票證） | JSON POST（三層結構） | 純字串 `1|OK` |
| AES-JSON（發票） | API 主動查詢（非 callback） | — |
