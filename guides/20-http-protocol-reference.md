...
> **重要**：此公式使用 **Data 明文 JSON**（加密前），直接串接 HashKey 值 + JSON + HashIV 值（無 `=` 或 `&` 分隔符），不是 AIO 金流的「所有欄位排序串接」方式。兩者不可混用。詳見 [guides/09 §CheckMacValue 計算](./09-ecticket.md)。
