...
> ⚠️ `Factory::createWithHash()` 已於 v1.0.2105270 棄用，請改用建構子傳入 hashKey/hashIv：
...
**FormWithCmvService 使用範例**（用於需要 CheckMacValue 的表單 POST，如物流建單）：
...
| `AesRequest::class` | 產生 AES 加密的回應（PHP 範例中常 alias 為 `AesGenerater`）|

> **注意**：部分 PHP 範例使用 `use Ecpay\Sdk\Response\AesJsonResponse as AesParser` 和 `use Ecpay\Sdk\Request\AesRequest as AesGenerater` 作為別名。實際 SDK 類別為 `AesJsonResponse` 和 `AesRequest`。
...
| ECPG 站內付 | SHA256（不影響） | PostWithAesJsonResponseService |
...
| ECPG GetToken | `CreateCreditOrder/GetToken.php` | `CreateAllOrder/GetToken.php` | 指定信用卡的 Token 取得（主範例支援全付款方式）。**GetToken 是 ECPG SDK 的內部流程，由 Service 類自動呼叫，開發者通常無需直接使用。** |
| 物流更新 | `UpdateFamiCvsStore.php` | `UpdateShipmentInfo.php` | 特定超商的門市更新變體 |
