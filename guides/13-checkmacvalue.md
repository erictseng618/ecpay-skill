...
4. UrlService::ecpayUrlEncode()— urlencode → 轉小寫 → .NET 特殊字元替換
...
以下實作從 PHP SDK 原始碼精確翻譯。支援 Python、Node.js、Java、C#、Go、C、C++、Rust、Swift、Kotlin、Ruby，每種語言提供三個函式：
...
public static class EcpayCheckMacValue
{
    ...
    {
        string encoded = HttpUtility.UrlEncode(source); // 空格→+
        encoded = encoded.ToLower();
        return encoded
            .Replace("%2d", "-").Replace("%5f", "_").Replace("%2e", ".")
            .Replace("%21", "!").Replace("%2a", "*")
            .Replace("%28", "(").Replace("%29", ")")
            .Replace("'", "%27")   // .NET Framework 不編碼 '，手動補上
...
>
> **`.NET 版本差異`**：`.NET Framework` 的 `HttpUtility.UrlEncode` 不編碼 `'`（單引號），
> 而 `.NET 6+` 會編碼為 `%27`。上方程式碼的 `.Replace("'", "%27")` 已同時處理兩個版本。
...
// URL encode（手動實作，空格→+）
// 注意：PHP urlencode() 會編碼 -_.，再由 ecpayUrlEncode 替換回來
