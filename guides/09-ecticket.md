...
- **PHP SDK 範例**：官方 PHP SDK 未包含電子票證範例。本指南提供的 PHP 程式碼片段基於 SDK 的 `PostWithAesJsonResponseService`，與發票/全方位物流的 AES-JSON 呼叫方式相同，可參考 `scripts/SDK_PHP/example/Invoice/B2C/` 內的範例（如 `IssueInvoice.php`）作為 AES-JSON 三層結構的實作參考
