...
| OpenAI Custom GPTs | ChatGPT Plus / Pro / Business / Enterprise / Edu | [GPT 編輯器](https://chatgpt.com/gpts/editor) |
| OpenClaw | OpenClaw 安裝（開源免費，需 Node ≥22） | [安裝文件](https://docs.openclaw.ai/install) |

## 這是什麼？

ECPay Skill 是一個 **AI Skill 套件**——安裝到 AI 程式開發助手（Claude Code、GitHub Copilot CLI、OpenClaw、**OpenAI Custom GPTs** 等）後，AI 就能根據你的需求，直接生成綠界 API 串接程式碼、診斷錯誤、引導完整串接流程。
...
| **需要額外付費嗎？** | 本 Skill 免費提供參考使用（版權所有，詳見 [LICENSE](LICENSE)）。唯一費用是 AI 程式開發助手本身的訂閱費（見上方「前置需求」表格），以及綠界的交易手續費 |
...
| 術語 | 白話解釋 |
|------|---------|
| **MerchantID** | 你的商店編號（測試用 `3002607`，無需申請、立即可用） |
...
以下是安裝 Skill 後，在 AI 助手中的實際互動範例：

```
👤 你：幫我用 Python 串接 ECPay AIO 信用卡一次付清

🤖 AI：好的，我來幫你串接 ECPay AIO 信用卡付款。

    📋 我會為你生成：
    1. CheckMacValue 計算函式（SHA256 + URL encode）
    2. 建立訂單的 HTML 表單（自動 POST 至綠界）
    3. Callback 驗證處理（Flask 路由）
    4. 測試帳號設定（MerchantID: 3002607）

    ⚠️ 注意事項：
    - ReturnURL 是伺服器背景通知，ClientBackURL 是瀏覽器跳轉，不要搞混
    - RtnCode 是字串 "1"，不是數字 1
    - 測試環境設定 SimulatePaid=1 可免刷卡

    [以下是完整的 Python 程式碼...]
```

> 💡 AI 會自動讀取 guides/ 整合指南 + references/ 最新 API 規格 + lang-standards/ 語言規範，產出符合最佳實踐的完整程式碼。
...
| 服務 | MerchantID | 用途 |
|------|-----------|------|
| **金流（AIO / ECPG）** | `3002607` | 信用卡、ATM、超商代碼等全付款方式測試 |
| **電子發票** | `2000132` | B2C / B2B 發票開立測試 |
| **物流 B2C / 宅配** | `2000132` | 超商取貨 / 宅配測試 |
...
本著作之一切著作權及智慧財產權歸綠界科技股份有限公司所有。未經書面授權，不得重製、修改或散布。
